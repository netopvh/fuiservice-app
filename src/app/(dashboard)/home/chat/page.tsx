...
export default function ChatPage() {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Chat com Prestadores'))
    });

    const isDark = useAppSelector((state: RootState) => state.themeConfig.theme === 'dark' || state.themeConfig.isDarkMode);
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            <div className="flex-none">
                                <img src="/images/profile-34.jpeg" className="h-12 w-12 rounded-full object-cover" alt="" />
                            </div>
                            <div className="mx-3">
                                <p className="mb-1 font-semibold">{user?.data.name}</p>
                                <p className="text-xs text-white-dark">{user?.data.company_name}</p>
                            </div>
                        </div>
                    </div>
                    <div className="relative">
                        <input type="text" className="peer form-input ltr:pr-9 rtl:pl-9" placeholder="Pesquisar..." value={searchUser} onChange={(e) => setSearchUser(e.target.value)} />
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center">
                                                    <div className="relative flex-shrink-0">
                                                        <img src={`/images/${person.path}`} className="h-12 w-12 rounded-full object-cover" alt="" />
                                                        {person.active && (
                                                            <div>
                    ...
                    {!isShowUserChat && (
                        <div className="relative flex h-full items-center justify-center p-4">
                            ...
                            <div className="flex flex-col items-center justify-center py-8">
                                ...
                                <p className="mx-auto flex max-w-[400px] justify-center rounded-md bg-white-dark/20 p-2 font-semibold">
                                    <IconMessage className="ltr:mr-2 rtl:ml-2" />
                                    Clique no usuário para iniciar uma conversa
                                </p>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="flex items-center justify-between p-4">
                                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                                    ...
                                    <div className="relative flex-none">
                                        <img src={`/images/${selectedUser.path}`} className="h-10 w-10 rounded-full object-cover sm:h-12 sm:w-12" alt="" />
                                        <div className="absolute bottom-0 ltr:right-0 rtl:left-0">
                                            <div className="h-4 w-4 rounded-full bg-success"></div>
                                        </div>
                                    </div>
                                                        ...
                                                        <div className={`flex items-start gap-3 ${selectedUser.userId === message.fromUserId ? 'justify-end' : ''}`}>
                                                            <div className={`flex-none ${selectedUser.userId === message.fromUserId ? 'order-2' : ''}`}>
                                                                {selectedUser.userId === message.fromUserId ? (
                                                                    <img src={`/images/profile-34.jpeg`} className="h-10 w-10 rounded-full object-cover" alt="" />
                                                                ) : (
                                                                    ''
                                                                )}
                                                                {selectedUser.userId !== message.fromUserId ? (
                                                                    <img src={`/images/${selectedUser.path}`} className="h-10 w-10 rounded-full object-cover" alt="" />
                                                                ) : (
                                                                    ''
                                                                )}
                                                            </div>
